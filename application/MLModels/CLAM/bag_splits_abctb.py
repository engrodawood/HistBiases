...
import sys
sys.path.append('.')
from application.project import PROJECT_DIR,DATA_DIR,WORKSPACE_DIR,OUTPUT_DIR,CLAM_DICT_EXTRV_ABCTB,TISSUE_TYPES_HIST
...
for Repr in CLAM_DICT_EXTRV_ABCTB.keys():
    IN_FEATS_DIR = CLAM_DICT_EXTRV_ABCTB[Repr]['IN_FEATS_DIR']
    OUT_BAGS_DIR = CLAM_DICT_EXTRV_ABCTB[Repr]['OUT_BAGS_DIR']
    OUT_SPLITS_DIR = CLAM_DICT_EXTRV_ABCTB[Repr]['OUT_SPLITS_DIR']
    for tissue in ['brca']:#TISSUE_TYPES_HIST:
        ...
        npyDf = pd.DataFrame([g for g in glob(os.path.join(IN_FEATS_DIR, "*feat.npy"))],
                             columns = ['Path'])
        ...
        folds = CLAM_DICT_EXTRV_ABCTB[Repr]['folds']
         # Generating Labels
