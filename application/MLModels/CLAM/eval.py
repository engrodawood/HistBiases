...
parser.add_argument('--csv_path', type=str, default='E:/Ruoyu/Data/Sheffield/Test/Test_7/sheffield_tumor_normal_clean.csv')
parser.add_argument('--finetune_feat', default="False", help='True for finetuned features, False for ImageNet features.')
...
args.save_dir = os.path.join(args.output_dir, str(args.save_exp_code))
...
def save_time_stats(args, time_stats):
    # time_stats['mean_std'] = {}

    stat_fold = []
    for key in time_stats:
        stat_fold.append(time_stats[key]['time'])
    
    time_stats['mean_std'] = f'{round(np.mean(stat_fold), 4)}±{round(np.std(stat_fold), 4)}'

    time_df = pd.DataFrame.from_dict(time_stats)
    time_df.to_csv(os.path.join(args.save_dir, 'time.csv'))
    pass
...
if __name__ == "__main__":
    ...
    #save_time_stats(args, time_stats)
