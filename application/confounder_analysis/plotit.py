...
DIR = '/data/PanCancer/HistBiases/output/ConfounderAnalysis/labels_assoc_cv'

for tissue in TISSUE_TYPES_HIST:
    df = pd.read_csv(f'{DIR}/{tissue}.csv',index_col='Unnamed: 0')
    ...
    tickDf = pd.DataFrame()
    tickDf[['voi','xtick']] = [idx.split('_')[:2] for idx in df.index] 
    xticks = {'ticks':x,
            'labels':tickDf['xtick'].tolist(),
            'rotation':45
            }
    plt.xticks(**xticks)
    ...
    plt.yticks(**yticks)
    annotDf = tickDf.groupby('voi')['xtick'].count()
    annotDf = annotDf/annotDf.sum()
    fs=14
    # Here is the label and arrow code of interest
    idx = 0
    for mtick in annotDf.index:
        pos = float(annotDf.loc[mtick])
        plt.annotate(mtick, xy=((idx+pos/2), -0.15), xytext=((idx+pos/2), -0.25), xycoords='axes fraction', 
                    fontsize=fs, ha='center', va='bottom',
                    arrowprops=dict(arrowstyle=f'-[, widthB={pos*24}, lengthB=0.5', lw=1.0, color='k'))
        idx = idx+pos
    ...
    plt.tight_layout()
    # plt.legend()
    plt.savefig(f'{tissue}_temp.png')
