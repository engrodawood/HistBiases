...
CONFOUND_DIR = f'{OUTPUT_DIR}/ConfounderAnalysis/GRADE_UP'
experiments = get_experiments()
experiments_rev = dict(zip(experiments.values(),experiments.keys()))

OUT_DIR = f'{CONFOUND_DIR}/plots_temp'
mkdir(OUT_DIR)
...
for tissue in ['ucec']:#TISSUE_TYPES_HIST:
    ...
    # cols = list(set(df.index) - {'MSI','CIMP','CINGS','HM'})
    # df = df.loc[cols,:]

    # For colorectal mutations
    #df = df.loc[['TP53','BRAF','RNF43','ATM','APC','SOX9']]

    #df = df.loc[['MSI','CIMP','CINGS','HM'],:]

    # For endometrial
    df = df.loc[['PTEN','TP53','CTCF','ARID1A','KRAS','MGA'],:]
    # For BRCA
    #df = df.loc[['TP53','CDH1','MAP3K1','PIK3CA'],:]
    ...
    plt.rcParams['text.color'] = 'black'  # Change 'green' to your desired color
    ...
    offset_shift = {0:-0.25,1:0.3,2:0.5}
    for cidx in range(donuts_coords.shape[0]):
        ...
        text_offset = offset_shift[cidx%3]
        
        ax.annotate(text,xy=(xc+text_offset,yc),
                    xytext=(xc+text_offset,yc-0.032),#0.037
                    fontsize=(fsize-5), ha='center', va='center'  
                                )
        ax.annotate(pval,xy=(xc,yc),
                    xytext=(xc,yc+0.023),#0.02
                    fontsize=(fsize-3), ha='center', va='center'  
                                )
    yticks = {'ticks':[0.0,0.1,0.20,0.3,0.40,0.50,0.6,0.70,0.8,0.90,1.0],
            'labels':[f'{v:.2f}' for v in [0.0,0.1,0.20,0.30,0.40,0.50,0.6,0.70,0.8,0.90,1.0]],'fontsize':fsize-2}
    # yticks = {'ticks':[0.50,0.6,0.70,0.8,0.90,1.0],
    #         'labels':[f'{v:.2f}' for v in [0.50,0.6,0.70,0.8,0.90,1.0]],'fontsize':fsize-2}
    fontDict ={'size':fsize-1}
    plt.xticks(**d_xticks)
    plt.yticks(**yticks)
    plt.ylim(0.0,1.02)
    #plt.ylim(0.45,1.02)
    ...
    #x_off,xs_off,sfactor,ssfactor =0.016,0.002,1.8,0.9# BRCA grade TCGA
    #x_off,xs_off,sfactor,ssfactor =0.016,0.002,2,1.7# BRCA grade ABCTB
    x_off,xs_off,sfactor,ssfactor =0.02,0,1.55,0.70 # colon grade

    offset = 2
    xsshift,xeshift = 2.1,1.99##95
    xtt = np.linspace((annotMinor.min()+xsshift)/idx,(annotMinor.max()-xeshift)/idx,annotMinor.shape[0])
    for ids in range(len(annotMinor)):
        field = r"$\mathrm{TMB}_{\widetilde{" + genes[ids] + "}}$" if field=='TMB' else 'Grade'
        xt= annotMinor[ids]
        print(xt)
        ...
        plt.annotate(field, xy=(x1-xs_off, -0.052), xytext=(x1-xs_off, -0.10), xycoords='axes fraction', 
        fontsize=fsize-2, ha='center', va='bottom',
        arrowprops=dict(arrowstyle=f'-[, widthB={offset*ssfactor}, lengthB=0.2', lw=1.0, color='black'))# CRC grade offset *0.71
        
        plt.annotate(genes[ids], xy=(x1-x_off, -0.11), xytext=(x1-x_off, -0.165), xycoords='axes fraction', 
    ...
    lmx,lmy = -0.0249,-0.157#99
    plt.annotate('Prediction target:',fontweight='bold', xy=(lmx, lmy), xytext=(lmx, lmy), xycoords='axes fraction', 
            fontsize=fsize-4, ha='center', va='bottom', color='black')
    plt.tight_layout()

    plt.savefig(f'{OUT_DIR}/{cohort}_{tissue}_mutations.png',dpi=600,bbox_inches='tight')
