...
OUT_DIR = f'{CONFOUND_DIR}/plots_final'
mkdir(OUT_DIR)
...
for tissue in ['brca']:#TISSUE_TYPES_HIST:
    ...
    #df = df.loc[['ER','PR','HER2'],:]
    # df = df.loc[['MSI','CINGS','HM'],:]
    # df = df.loc[['TP53','CDH1','MAP3K1','PIK3CA'],:]
    #df = df.loc[['TP53','BRAF','RNF43','ATM','APC','SOX9'],:]
    sel_cols = ['PTEN','TP53','CTCF','ARID1A','KRAS']
    #sel_cols = ['ER','PR','HER2','TP53','CDH1']
    sel_cols = ['ER','PR','HER2']
    df = df.loc[sel_cols,:]
    # Reseting the index
    df = df.reset_index()
    ...
    for cidx in range(donuts_coords.shape[0]):
        ...
        #             xytext=(xc+text_offset,yc-0.019),#0.037
        #             fontsize=(fsize-5), ha='center', va='center'  
        #                         )
        ax.annotate(pval,xy=(xc,yc),
                    xytext=(xc,yc+0.028),#0.028#0.041
    ...
    x_off,xs_off,sfactor,ssfactor =0.03,0,1.8,0.95 # colon grade

    offset = 2
    xsshift,xeshift = 1.5,1.49##95
    ...
    plt.savefig(f'{OUT_DIR}/{cohort}_{tissue}_comb.png',dpi=600,bbox_inches='tight')
