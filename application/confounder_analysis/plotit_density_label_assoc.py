...
for tissue in TISSUE_TYPES_HIST:
    ...
    for key in df.index:
        ...
        for skidx,skey in enumerate(shift.keys()):
            ...
            if voi in markers:
                xticks.append(f'{markers[voi][skey]}')# voi
            else:
                xticks.append(f'{skey}') # voi
    ...
    x = list(set(xticks_v))# np.arange(idx+1)
    xticks = {'ticks':x,
              'labels':['All'  if tk not in list(shift.keys())+['+ve','-ve','H','L'] else tk for tk in xticks],
            #   'rotation':70
    }
    ...
    annotDf = df.groupby('PRED',sort=False)['PRED'].count()
    ...
    for i in x:
        if i in xticks_lines:
            vline = plt.axvline(x=i+1,ymin=0,ymax=1,linewidth=1,color='black')
            x_loc = vline.get_data()[0][0]
            relative_loc = ((x_loc-x_min)/x_range)
            if prev:
                dsf = (relative_loc-prev)/2
            else:
                dsf = relative_loc/2
            plt.annotate(annotDf.index[idx], xy=((relative_loc-dsf), -0.18), xytext=((relative_loc-dsf), -0.22), xycoords='axes fraction', 
            fontsize=fs, ha='center', va='bottom',
            arrowprops=dict(arrowstyle=f'-[, widthB={float(annotDf.iloc[idx])*26}, lengthB=0.2', lw=1.0, color='k'))
            prev = relative_loc
            idx+=1
        else:
            vline = plt.axvline(x=i,ymin=0,ymax=1,linestyle='dotted',linewidth=0.1,color='black')
    
    #The annotation is done based on the tick line. For last one we need to tack back
    #Though bad coding but for now its ok
    x_loc = vline.get_data()[0][0]
    relative_loc = ((x_loc-x_min)/x_range)
    dsf = (relative_loc-prev)/2
    plt.annotate(annotDf.index[idx], xy=((relative_loc-dsf), -0.18), xytext=((relative_loc-dsf), -0.22), xycoords='axes fraction', 
    ...
    plt.tight_layout()
    # plt.legend()
    plt.savefig(f'{OUT_DIR}/{tissue}.png')