...
CONFOUND_DIR = f'{OUTPUT_DIR}/ConfounderAnalysis/labels_assoc_cv_up'
...
for tissue in ['colon']:
    ...
    for key in df.index:
        ...
        ratios.append(df.loc[key,'MUT-MUT']/df.loc[key,['WT-MUT','MUT-MUT']].sum())
        # else:
        #     ratio_fields = [f'{f} ratio' for f in fields]
        ratios.append(df.loc[key,'MUT-WT']/df.loc[key,['WT-WT','MUT-WT']].sum())
        ...
        for skidx,skey in enumerate(shift.keys()):
            n_counts = df.loc[key,[f'{v}-{skey}' for v in ['WT','MUT']]].sum()*100
    ...
    for cidx in range(donuts_coords.shape[0]):
        ...
        text_offset = -0.25 if cidx % 2==0 else 0.25
        ax.annotate(text,xy=(xc+text_offset,yc),
                    xytext=(xc+text_offset,yc-0.027),
    ...
    plt.savefig(f'{OUT_DIR}/{cohort}_{tissue}.png',dpi=600)
